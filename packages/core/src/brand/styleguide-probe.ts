...
export function styleguideProbe(): Omit<Styleguide, "url"> {
  ...
  for (const el of elements) {
    ...
    for (const p of [s.paddingTop, s.paddingLeft, s.gap, s.marginBottom]) {
      if (p && p !== "0px" && p !== "normal" && p.endsWith("px"))
        spacing.set(p, (spacing.get(p) ?? 0) + 1);
    }
  }
  ...
  const button = Array.from(
    document.querySelectorAll("button, a[class*='btn' i], a[class*='button' i], [role='button']"),
  ).find(visible);
  const buttonStyle = button ? getComputedStyle(button) : null;
  const link =
    Array.from(document.querySelectorAll("main a, article a, p a")).find(visible) ??
  ...
  const primary =
    ...
    !isNeutral(toHex(buttonStyle.backgroundColor)!)
      ? toHex(buttonStyle.backgroundColor)
      : chromatic[0]?.[0]) ?? null;
  ...
  for (const sheet of Array.from(document.styleSheets)) {
    ...
    for (const rule of Array.from(rules)) {
      if (!(rule instanceof CSSStyleRule) || !/^(:root|html|body)$/.test(rule.selectorText))
        continue;
      for (const name of Array.from(rule.style)) {
        if (name.startsWith("--") && Object.keys(cssVariables).length < 200) {
          cssVariables[name] =
            rootStyle.getPropertyValue(name).trim() || rule.style.getPropertyValue(name).trim();
        }
      }
    }
  }
  ...
  for (const face of Array.from(document.fonts)) {
    if (!fontSources.has(face.family))
      fontSources.set(face.family.replace(/["']/g, ""), face.status);
  }
  for (const l of Array.from(
    document.querySelectorAll<HTMLLinkElement>(
      "link[href*='fonts.googleapis.com'], link[href*='use.typekit.net']",
    ),
  )) {
    for (const fam of l.href.matchAll(/family=([^&:]+)/g))
      fontSources.set(decodeURIComponent(fam[1]!).replace(/\+/g, " "), l.href);
  }
  ...
  return {
    ...
    colors: {
      ...
      palette: ranked
        .slice(0, 12)
        .map(([hex, v]) => ({ hex, usage: Math.round((v / total) * 1000) / 1000 })),
    },
    typography: {
      fonts: [...fonts.entries()]
        .filter(([f]) => f)
        ...
        .map(([family, usage]) => ({
          family,
          usage: [...usage],
          source: fontSources.get(family) ?? null,
        })),
      headings,
    ...
    components: {
      button: buttonStyle
        ...
        : null,
      link: link
        ? pick(getComputedStyle(link), ["color", "text-decoration-line", "font-weight"])
        : null,
      input: input
        ? pick(getComputedStyle(input), [
            "background-color",
            "color",
            "border",
            "border-radius",
