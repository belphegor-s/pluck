...
import { z } from "zod";
var httpUrl = z.url({ protocol: /^https?$/ }).max(4096).meta({ description: "Absolute http(s) URL.", example: "https://example.com" });
var domain = z.string().trim().toLowerCase().min(3).max(253).transform((v) => v.replace(/^https?:\/\//, "").replace(/[/?#].*$/, "").replace(/^www\./, "")).pipe(z.string().regex(z.regexes.domain, "Invalid domain")).meta({ description: "Domain name, e.g. stripe.com", example: "stripe.com" });
var proxyMode = z.enum(["auto", "none", "datacenter", "residential"]).default("auto").meta({
  description: "Egress strategy. `auto` goes direct and escalates through datacenter then residential proxies when a request is blocked."
});
var renderMode = z.enum(["auto", "always", "never"]).default("auto").meta({
  description: "Headless browser usage. `auto` fetches over HTTP first and only renders when the page needs JavaScript."
});
var llmOverride = z.object({
  provider: z.enum(["openrouter", "openai", "anthropic", "google", "groq", "openai-compatible"]).optional(),
  model: z.string().max(200).optional()
}).optional().meta({
  description: "Pick the model used for AI steps. Supply your own key via the `x-llm-key` header to pay the provider directly."
});
...
var brand = z2.object({
  ...
  industries: z2.object({ naics: z2.array(industryCode), sic: z2.array(industryCode) }).nullable(),
  isNsfw: z2.boolean(),
  updatedAt: z2.string()
}).meta({ id: "Brand" });
var classifyRequest = z2.object({
  domain: domain.optional(),
  description: z2.string().min(10).max(8e3).optional(),
  llm: llmOverride
}).refine((v) => v.domain || v.description, { message: "Provide `domain` or `description`." }).meta({ id: "ClassifyRequest" });
var classifyResult = z2.object({
  naics: z2.array(industryCode),
  sic: z2.array(industryCode)
});
var transactionRequest = z2.object({
  ...
  mcc: z2.string().regex(/^\d{4}$/).optional(),
...
var config = {
  ...
  description: "Pluck turns any URL into clean markdown, structured JSON, screenshots and brand data for AI. Open source, self-hostable, and you can bring your own model key.",
...
var browserAction = z3.discriminatedUnion("type", [
  ...
  z3.object({ type: z3.literal("type"), selector: z3.string().max(500), text: z3.string().max(2e3) }),
...
var scrapeOptions = z3.object({
  formats: z3.array(scrapeFormat).min(1).max(9).default(["markdown"]).meta({ description: "Output formats to return." }),
  onlyMainContent: z3.boolean().default(true).meta({ description: "Strip nav, footers, cookie banners and ads before conversion." }),
  ...
  proxy: proxyMode,
  country: z3.string().length(2).toLowerCase().optional().meta({ description: "ISO country code for geo-targeted proxies." }),
  ...
  jsonOptions: z3.object({
    ...
    llm: llmOverride
  }).optional().meta({ description: "Used with the `json` format to extract structured data." })
});
...
var scrapeResult = z3.object({
  ...
  screenshot: z3.string().optional().meta({ description: "Public URL, or a data URI when storage is not configured." }),
...
var screenshotRequest = screenshotOptions.extend({
  url: httpUrl,
  waitFor: z3.number().int().min(0).max(3e4).optional(),
  actions: z3.array(browserAction).max(25).optional(),
  proxy: proxyMode,
...
var parseRequest = z3.object({
  url: httpUrl.optional(),
  base64: z3.string().max(4e7).optional().meta({ description: "File bytes, base64-encoded (max ~30MB)." }),
...
var mapRequest = z3.object({
  url: httpUrl,
  search: z3.string().max(200).optional().meta({ description: "Rank URLs by relevance to this text." }),
...
// ../../packages/shared/dist/jobs.js
var webhookConfig = z4.object({
  url: httpUrl,
  events: z4.array(z4.enum(["started", "page", "completed", "failed"])).default(["completed", "failed"])
}).meta({ description: "Signed POST callbacks. Verify with the `pluck-signature` header." });
var crawlRequest = z4.object({
  url: httpUrl,
  limit: z4.number().int().min(1).max(1e4).default(100),
  maxDepth: z4.number().int().min(0).max(20).default(3),
  includePaths: z4.array(z4.string().max(300)).max(50).optional().meta({ description: "Glob patterns, e.g. `/blog/**`." }),
...
var searchRequest = z4.object({
  ...
  scrape: scrapeOptions.pick({
    ...
    maxAge: true,
    timeout: true
  }).optional().meta({ description: "Also scrape every result. Scrape credits apply per result." })
...
var extractRequest = z4.object({
  url: httpUrl,
  schema: z4.record(z4.string(), z4.unknown()).optional().meta({ description: "JSON Schema the result must satisfy." }),
  prompt: z4.string().max(8e3).optional(),
  render: scrapeOptions.shape.render,
  proxy: proxyMode,
  maxAge: scrapeOptions.shape.maxAge,
  llm: llmOverride
}).refine((v) => v.schema || v.prompt, { message: "Provide `schema`, `prompt`, or both." }).meta({ id: "ExtractRequest" });
var extractResult = z4.object({ url: z4.string(), data: z4.unknown() });
var product = z4.object({
  name: z4.string(),
...
var productRequest = z4.object({ url: httpUrl, proxy: proxyMode, maxAge: scrapeOptions.shape.maxAge, llm: llmOverride }).meta({ id: "ProductRequest" });
var productsRequest = productRequest.extend({ limit: z4.number().int().min(1).max(500).default(50) }).meta({ id: "ProductsRequest" });
...
var styleguideRequest = z4.object({ url: httpUrl, proxy: proxyMode, maxAge: scrapeOptions.shape.maxAge }).meta({ id: "StyleguideRequest" });
var styleguide = z4.object({
  url: z4.string(),
  ...
  typography: z4.object({
    fonts: z4.array(z4.object({ family: z4.string(), usage: z4.array(z4.string()), source: z4.string().nullable() })),
    headings: z4.record(z4.string(), z4.object({
      fontFamily: z4.string(),
      fontSize: z4.string(),
      fontWeight: z4.string(),
      lineHeight: z4.string()
    })),
    body: z4.object({ fontFamily: z4.string(), fontSize: z4.string(), lineHeight: z4.string() }).nullable()
  }),
...
var monitorCreate = z4.object({
  name: z4.string().min(1).max(120).optional(),
  type: monitorType,
  url: httpUrl,
  intervalMinutes: z4.number().int().min(5).max(43200).default(1440),
  webhook: httpUrl.optional(),
  selector: z4.string().max(500).optional().meta({ description: "`page` only: watch a single CSS selector." }),
  schema: z4.record(z4.string(), z4.unknown()).optional().meta({ description: "`extract` only." }),
...
var usage = z5.object({
  ...
  byEndpoint: z5.array(z5.object({ endpoint: z5.string(), requests: z5.number().int(), credits: z5.number().int() })),
  daily: z5.array(z5.object({ date: z5.string(), requests: z5.number().int(), credits: z5.number().int() }))
});
...
var endpoints = {
  scrape: ep({
    ...
    description: "Fetch any page and return clean, LLM-ready markdown plus optional HTML, links, images, metadata, a screenshot or schema-validated JSON. JavaScript is rendered automatically when needed.",
...
var tools = Object.values(endpoints).filter(
  (e) => Boolean(e.mcp)
);
...
var toStructured = (data) => data && typeof data === "object" && !Array.isArray(data) ? data : { result: data };

// src/http.ts
var apiUrl = process.env.PLUCK_API_INTERNAL_URL ?? process.env.PUBLIC_API_URL ?? "http://localhost:8080";
var KEY_HINT = `${BRAND.apiKeyLive}\u2026`;
var app = new Hono();
app.use(
  "*",
  cors({
...
app.get(
  "/",
  (c) => c.json({ name: `${BRAND.name} MCP`, endpoint: "/mcp", docs: `${BRAND.siteUrl}/docs/mcp` })
);
...
serve(
  ...
  (info) => console.log(`pluck mcp listening on ${info.port}`)
);
