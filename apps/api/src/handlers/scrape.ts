...
import { PluckError, type ScrapeRequest, type ScrapeResult, credits, scrapeCost } from "@pluck/shared";
...
const estimateScrape = (req: ScrapeRequest) =>
  scrapeCost({
    browser: req.render === "always" || req.formats.includes("screenshot") || Boolean(req.actions?.length),
...
export const parse = handler("parse", {
  ...
  async run({ s }, req) {
    ...
    if (req.url) {
      ...
      if (res.status >= 400) throw new PluckError("target_unreachable", `Document responded with HTTP ${res.status}.`);
      body = res.body;
      contentType ??= res.contentType;
    } else {
      body = Buffer.from(req.base64!, "base64");
    }
    const data = await parseDocumentBytes(body, { contentType, filename: req.filename ?? req.url?.split("/").pop() });
...
export const map = handler("map", {
  ...
  async run({ s }, req) {
    ...
    if (hit && Date.now() - hit.storedAt < 3_600_000) return { data: hit.value, credits: credits.map, cached: true };
...
export const screenshot = handler("screenshot", {
  estimate: (i) => credits.scrape + credits.browserRender + credits.screenshot + (i.proxy === "residential" ? credits.residentialProxy : 0),
  target: (i) => i.url,
  async run({ s }, req) {
    const { maxAge, timeout: _t, ...identity } = req;
    const key = cacheKey("shot", identity);
    type Shot = { url: string; screenshot: string; width: number; height: number; format: "png" | "jpeg" | "webp" };
    if (maxAge > 0 && s.store) {
      const hit = await s.cache.get<Shot>(key);
      if (hit && Date.now() - hit.storedAt <= maxAge * 1000) return { data: hit.value, credits: credits.scrape, cached: true };
    }

    const { result, usage } = await s.scraper().scrape({
      url: req.url,
      formats: ["screenshot"],
      screenshot: { fullPage: req.fullPage, format: req.format, quality: req.quality, viewport: req.viewport },
      waitFor: req.waitFor,
      actions: req.actions,
      proxy: req.proxy,
      timeout: req.timeout,
    ...
    if (!result.screenshot) throw new PluckError("target_unreachable", "The page could not be captured.");
    const data: Shot = {
