...
import { absolute, decodeBody, extractBrandFromHtml, normaliseHex, parseDocument, registrableDomain } from "@pluck/core";
...
const FREE_MAIL = new Set([
  ...
  "web.de", "yandex.com", "yandex.ru", "mail.ru", "zoho.com", "fastmail.com", "hey.com", "tutanota.com", "qq.com",
  "163.com", "126.com", "rediffmail.com",
]);

const NOT_OFFICIAL = /(^|\.)(wikipedia\.org|linkedin\.com|facebook\.com|x\.com|twitter\.com|instagram\.com|youtube\.com|crunchbase\.com|bloomberg\.com|reuters\.com|yahoo\.com|google\.com|reddit\.com|glassdoor\.com|indeed\.com|marketwatch\.com|cnbc\.com|forbes\.com|nasdaq\.com|nyse\.com)$/;
...
export class BrandService {
  ...
  async resolveDomain(q: BrandQuery): Promise<string> {
    ...
    if (q.email) {
      ...
      if (FREE_MAIL.has(host)) throw new PluckError("bad_request", "That is a personal email provider, not a company domain.");
      return registrableDomain(`https://${host}`) || host;
    }
    const query = q.ticker ? `${q.ticker} stock company official website investor relations` : `${q.name} official website`;
  ...
  async get(domain: string, opts: { maxAge: number; proxy: BrandQuery["proxy"]; llm: LlmTasks | null }): Promise<{ brand: Brand; cached: boolean }> {
    ...
    for (const icon of manifest?.icons ?? []) {
      ...
      if (url && !logos.some((l) => l.url === url)) {
        logos.push({ url, type: "icon", format: icon.type?.split("/")[1] ?? null, width: size ? Number(size[1]) : null, height: size ? Number(size[2]) : null, theme: null });
      }
    }

    const colors = [...new Set([normaliseHex(manifest?.theme_color), ...stat.colors].filter((c): c is string => Boolean(c)))];
