...
export default async function ProfilePage() {
  const { user } = await requireWorkspace("/dashboard/profile");
    ...
    <div className="space-y-12">
      ...
      <section className="space-y-4 border border-[color-mix(in_srgb,var(--accent)_45%,var(--line))] p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--ink-soft)]">
            Deleting your account removes every workspace you are the only member of, straight away,
            with its keys, monitors and credits. Workspaces with other people carry on without you;
            if you are the only owner of one, hand it over first.
          </p>
        </div>
